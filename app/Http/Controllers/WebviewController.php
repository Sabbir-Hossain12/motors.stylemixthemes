...
class WebviewController extends Controller
{
    ...
    public function datafeed()
    {
        ...
        $channel->addChild('title', 'Premiershopbd');
        $channel->addChild('link', url('https://www.premiershopbd.com/'));
        $channel->addChild('description', 'Premiershopbd is an online luxury store offering a wide range of premium bags and accessories, including leather handbags, backpacks, and clutches. The site features elegant designs for both men and women and regularly provides discounts on select items.');
        $idnew=0;
            ...
            if (count($products) > 0) {
                foreach ($products as $indexs => $product) {
                    if (isset($product)) {
                        ...
                        $item->addChild('g:link', 'https://premiershopbd.com/product/' . $product->slug);
                        $item->addChild('g:image_link', 'https://premiershopbd.com/' . $product->ProductImage);
                        $item->addChild('g:brand', 'premiershopbd');
...
//         dd($categoryproducts);
    ...
    public function mainview2()
    {
        ...
        $topproducts = Mainproduct::where('status', 'Active')->where('top_rated', '1')->orderByRaw('ISNULL(`position`), `position` ASC')->select('id', 'ProductName', 'ProductSlug', 'ProductImage', 'status', 'position', 'top_rated', 'RelatedProductIds')->inRandomOrder()->take(8)->get();
        $categoryproducts = Category::where('status', 'Active')->where('front_status', 1)->select('id', 'category_name', 'slug', 'position')->orderBy('position')->get();
        ...
        return view('webview.content.maincontent', ['categories' => $categories, 'sliders' => $sliders, 'adds' => $adds, 'addbottoms' => $addbottoms, 'topproducts' => $topproducts, 'categoryproducts' => $categoryproducts]);
    }
